      ...
      <div class="row mb-4">
        ...
        <div class="col-12 col-md-6 mb-3">
          <div class="card text-white bg-success mb-3">
            <div class="card-body">
              <h5 class="card-title">Infraestructura pendiente</h5>
              <p class="card-text display-6">{{ infraestructuraFueraServicio }}</p>
            </div>
          </div>
        </div>
...
export default {
  data() {
    return {
      ...
      reportesHoy: 0,
      infraestructuraFueraServicio: 0,
    };
  },
  async mounted() {
    this.centrosActivos = await obtenerCentrosActivos();
    this.solicitudesActivas = await obtenerSolicitudesActivas();
     this.infraestructuraFueraServicio = await obtenerInfraestructuraFueraServicio();
  },
};
</script>
